  ...
  try {
    ...
    for (const item of items) {
      ...
      while (hasMore) {
        ...
        const addOps = added.map((tx) =>
          prisma.transaction.upsert({
            ...
            create: {
              ...
              category: (tx.category as unknown as Prisma.InputJsonValue) ?? undefined,
              location: (tx.location as unknown as Prisma.InputJsonValue) ?? undefined,
        ...
        const delOps = removed.map((tx) =>
          prisma.transaction.deleteMany({ where: { plaidTransactionId: tx.transaction_id } })
        );
